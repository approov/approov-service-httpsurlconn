...
- Automatic release tagging on merge to `main` (`tag-release.yml`): the top CHANGELOG entry drives a matching git tag, which triggers the Maven publish workflow. Skipped if the tag already exists.
